import {timestampSeconds} from './liveCandleTime';
...
export interface LiquidityReport {
  ...
  interval?:string; intervalSeconds?:number; priceDecimals?:number;
  spread?: number; buyVolume?: number; sellVolume?: number; tradeWindow?: number;
}

export function analyzeCandles(input: LiquidityCandle[], now = Date.now(), intervalSeconds = 300) {
  const unique = new Map<number, LiquidityCandle>();
  for (const c of input) {
    if ([c.time,c.open,c.high,c.low,c.close].every(Number.isFinite) && c.time>0 && timestampSeconds(c.time)*1000<=now && c.low > 0 && c.high >= Math.max(c.open,c.close,c.low) && c.low <= Math.min(c.open,c.close)) unique.set(timestampSeconds(c.time),{...c,time:timestampSeconds(c.time)});
  }
  const candles = [...unique.values()].sort((a,b)=>a.time-b.time).slice(-288);
  if (candles.length < 3) throw new Error('Insufficient valid candle history');
  const last = candles[candles.length-1];
  const closed = candles.filter(c=>(c.time+intervalSeconds)*1000 <= now);
  ...
  const sweep = trigger.high > high && trigger.close < high ? 'Upper range sweep candidate (closed candle)' : trigger.low < low && trigger.close > low ? 'Lower range sweep candidate (closed candle)' : 'No closed-candle range sweep detected';
  ...
  for (const c of closed) {
    ...
    bins[index].size += hasVolume ? Math.max(0,Number.isFinite(c.volume)?c.volume!:0) : 1;
  }
  // Repeated nearby swing extremes are interaction clusters, never resting orders.
  const tolerance=closed.slice(-20).reduce((sum,c)=>sum+c.high-c.low,0)/Math.max(1,Math.min(20,closed.length))*.15;
  const clusters:LiquidityLevel[]=[];
  for(let i=1;i<closed.length-1;i++) {
    const c=closed[i];
    for(const [price,side,isSwing] of [[c.high,'Swing-high cluster',c.high>closed[i-1].high&&c.high>=closed[i+1].high],
      [c.low,'Swing-low cluster',c.low<closed[i-1].low&&c.low<=closed[i+1].low]] as const) {
      if(!isSwing)continue;
      const cluster=clusters.find(l=>l.side===side&&Math.abs(l.price-price)<=tolerance);
      if(cluster){cluster.price=(cluster.price*cluster.size+price)/(cluster.size+1);cluster.size++;}
      else clusters.push({price,size:1,side});
    }
  }
  const nearby=clusters.filter(l=>l.size>=2&&Math.abs(l.price-high)>tolerance&&Math.abs(l.price-low)>tolerance)
    .sort((a,b)=>Math.abs(a.price-last.close)-Math.abs(b.price-last.close)).slice(0,4);
  ...
  return {price:last.close,marketTime:last.time*1000, sweep,
    ...
    levels:[{price:high,size:0,side:'Prior 20-bar high'},{price:low,size:0,side:'Prior 20-bar low'},...nearby],
