...
export function LiquidityReader({onClose}:{onClose:()=>void}) {
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      ...
      try {
        const response=await fetch(`/api/liquidity?symbol=${symbol}`,{signal:controller.signal});
      ...
      finally{if(!cancelled){setLoading(false);timer=setTimeout(poll,symbol==='BTCUSDT'?5000:30000);}}
    };
  ...
  return <section role="dialog" aria-modal="true" aria-label={text('Liquidity Reader')} dir={dir} className="fixed inset-0 z-[110] overflow-y-auto bg-[#0B0F17] text-slate-200">
    <div className="max-w-6xl mx-auto p-4 sm:p-8">
      ...
      <nav aria-label={text('Liquidity instruments')} className="flex flex-wrap gap-2 my-6">{instruments.map(([id,label])=><button key={id} aria-pressed={id===symbol} onClick={()=>setSymbol(id)} className={`px-4 py-2 rounded-lg border text-sm ${id===symbol?'bg-amber-400 text-slate-950 border-amber-400':'border-slate-700 text-slate-300'}`}>{text(label)} <span dir="ltr" className="text-xs opacity-70">{id}</span></button>)}</nav>
      ...
      {data && <>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5"><div><span className={`text-xs px-2 py-1 rounded border ${data.kind==='book'?'text-emerald-300 border-emerald-800':'text-amber-300 border-amber-800'}`}>{data.kind==='book'?text('REAL ORDER BOOK'):text('CALCULATED / ESTIMATED')}</span><p className="text-xs text-slate-400 mt-3">{text(data.source)}</p></div><div className="text-xs text-slate-400 text-end">{text('Snapshot')} {new Date(data.observedAt).toLocaleTimeString(locale)}<br/>{text('Market timestamp')} {data.marketTime?new Date(data.marketTime).toLocaleString(locale):text('Unavailable')}{Date.now()-data.marketTime>(data.kind==='book'?60000:600000)&&<span className="block text-amber-400">{text('Market/feed inactive or delayed')}</span>}</div></div>
