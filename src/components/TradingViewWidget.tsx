...
import { registerReactionZoneTools, setReactionZoneMarketData, clearReactionZoneDrawings } from './chart/reactionZoneManager';
...
const loadStrategyDrawingsLocal = (sym: string, strategy?: string | null): any[] => {
  try {
    ...
    if (sym.includes(':')) {
      ...
      // Cross-asset alias fallback for Gold across brokers (e.g. GOLD vs XAUUSD)
      if (baseTicker === 'GOLD') {
        const xauKey = getStrategyStorageKey('XAUUSD', strategy);
        const xauRaw = localStorage.getItem(xauKey);
        if (xauRaw) {
          const parsed = JSON.parse(xauRaw);
          const sanitized = sanitizeDrawingsList(parsed);
          if (sanitized.length > 0) return sanitized;
        }
      } else if (baseTicker === 'XAUUSD') {
        const goldKey = getStrategyStorageKey('GOLD', strategy);
        const goldRaw = localStorage.getItem(goldKey);
        if (goldRaw) {
          const parsed = JSON.parse(goldRaw);
          const sanitized = sanitizeDrawingsList(parsed);
          if (sanitized.length > 0) return sanitized;
        }
      }
...
export const TradingViewWidget: React.FC<TradingViewWidgetProps> = memo(({
      ...
      if (rawDrawings.length > 0) {
        ...
        rawDrawings.forEach((d: SerializedDrawingPayload) => {
          ...
          try {
            ...
            if ((d.options as any)?.zoneType === 'strong' || (d as any).zoneType === 'strong' || actualType === 'reaction-zone-strong') {
              actualType = 'reaction-zone-strong';
            } else if ((d.options as any)?.zoneType === 'weak' || (d as any).zoneType === 'weak' || actualType === 'reaction-zone-weak') {
              actualType = 'reaction-zone-weak';
            } else if (!registry.has(actualType)) {
  ...
  const brokerName =
    ...
    rawBroker === 'FOREXCOM' ? 'FOREX.com' :
    rawBroker === 'PEPPERSTONE' ? 'Pepperstone' :
    rawBroker === 'FXCM' ? 'FXCM' :
    rawBroker === 'CAPITALCOM' ? 'Capital.com' :
    rawBroker === 'ICMARKETS' ? 'IC Markets' :
    rawBroker === 'SAXO' ? 'Saxo Bank' :
    rawBroker === 'TVC' ? 'TVC' :
    rawBroker === 'BINANCE' ? 'Binance' :
    rawBroker === 'BYBIT' ? 'Bybit' :
