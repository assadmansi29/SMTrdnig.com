  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
      <div className="bg-[#0D121F] border border-slate-700/80 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden text-slate-200">
        {/* Search Input Bar */}
        <div className="relative p-4 border-b border-slate-800 bg-[#090D17] flex items-center">
          <Search className="w-5 h-5 text-amber-400 mr-3" />
          <input
            type="text"
            autoFocus
            placeholder="Search strategy models, indicators, assets (e.g. Order Flow, ES, Gold, Python)..."
            value={query}
        ...
        <div className="px-5 py-2.5 bg-[#0A0E18] border-b border-slate-800/80 flex flex-wrap items-center gap-2 text-xs">
          <span className="text-slate-400 flex items-center gap-1">
            <Tag className="w-3 h-3 text-slate-500" /> Hot Topics:
          </span>
          {['Order Flow', 'Macro Liquidity', 'Python StatArb', 'Gold XAU/USD', '0DTE Gamma', 'Psychology'].map(tag => (
            <button
              key={tag}
              onClick={() => setQuery(tag)}
              className="bg-slate-900 hover:bg-slate-800 text-slate-300 px-2 py-0.5 rounded border border-slate-800 text-[11px] transition-colors"
            >
              {tag}
            </button>
          ))}
        ...
        <div className="max-h-[60vh] overflow-y-auto p-4 space-y-2">
          {filteredArticles.length === 0 ? (
            <div className="text-center py-12 text-slate-500 text-sm">
              No quantitative models found matching "{query}".
            </div>
          ) : (
            filteredArticles.map((art) => (
              ...
              >
                ...
                <ArrowRight className="w-4 h-4 text-slate-600 group-hover:text-amber-400 group-hover:translate-x-0.5 transition-all shrink-0 mt-2" />
              </div>
            ))
          )}
        </div>
