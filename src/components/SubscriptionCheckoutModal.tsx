      ...
      >
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-base sm:text-lg font-black text-white tracking-tight">{ui(" Checkout & Payment: ")}<span className={selectedPlan.isBestOffer ? 'text-amber-400' : 'text-emerald-400'}>{ui(selectedPlan.name)}</span>
                </h2>
                {selectedPlan.badge && (
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                    ...
                    {ui(selectedPlan.badge)}
                  </span>
                )}
              </div>
              <p className="text-xs text-slate-400">{ui(" Official SM Trading Pro Subscription Activation via USDT ")}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-5 bg-[#0C1220] border border-slate-800/90 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <div>
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
                    {selectedPlan.isSiteSubscription ? ui("Site Subscription Package") : ui("Premium Membership")}
                  </span>
                  <h3 className="text-lg font-black text-white mt-0.5">{ui(selectedPlan.name)}</h3>
                </div>
                <div className="text-right rtl:text-left">
                  <div className="text-2xl font-black text-white font-mono-num">{selectedPlan.priceDisplay}</div>
                  <span className="text-[11px] text-slate-400">{ui(selectedPlan.billingPeriod)}</span>
                </div>
              </div>

              {selectedPlan.id === 'monthly' && <MonthlyPromotionNotice />}
              <LiveTrainingNotice />
              {/* Inclusions List */}
              <div className="space-y-2.5">
                <span className="text-xs font-bold text-slate-300 block">{ui("Package Inclusions:")}</span>
                <ul className="space-y-2">
                  {selectedPlan.features.map((feat, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-xs text-slate-300">
                      <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                      <span className="leading-snug">{ui(feat)}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {selectedPlan.isBestOffer && (
                <div className="p-3.5 bg-gradient-to-br from-amber-500/15 via-[#111928] to-slate-900 border border-amber-500/40 rounded-xl space-y-2">
                  <div className="flex items-center gap-1.5 text-amber-400 font-bold text-xs">
                    ...
                    <span>{ui("Included Educational Masterclasses:")}</span>
                  </div>
                  <ul className="space-y-1.5 pl-1">
                    <li className="text-xs text-slate-200 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0" />
                      <strong>{ui("SMC Trading Course")}</strong>{ui(" (Full Institutional Curriculum) ")}</li>
                  </ul>
                </div>
              )}
              ...
              <div className="pt-2 border-t border-slate-800 text-[11px] text-slate-400 flex items-center gap-2">
                ...
                <span>{ui("Immediate account verification & manual desk onboarding")}</span>
              </div>
            </div>
              ...
              <div className="p-4 bg-gradient-to-r from-emerald-950/40 via-[#0E1526] to-slate-900 border border-emerald-500/40 rounded-2xl">
                <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-black text-white">{ui("Payment Method: ")}<span className="text-emerald-400 font-mono">USDT (TRC20)</span></span>
                  </div>
                  <div className="flex items-center gap-1.5 text-[10px] font-mono font-bold text-emerald-300 bg-emerald-500/15 px-2.5 py-0.5 rounded-full border border-emerald-500/40">
                    <span>{ui("USDT ONLY")}</span>
                    <span>•</span>
                    <span>{ui("NETWORK: TRC20")}</span>
                  </div>
                </div>
                
                <p className="text-xs text-slate-300 leading-relaxed">{ui(" Payment must be ")}<strong>{ui("USDT ONLY")}</strong>{ui(". The ")}<strong>{ui("ONLY supported USDT network is TRC20")}</strong>.
                </p>
              </div>
              ...
              <div className="p-4 bg-slate-900/80 border border-slate-800 rounded-2xl space-y-3">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="space-y-2">
                    <h4 className="text-xs font-bold text-white uppercase tracking-wider">{ui("How To Complete Your Subscription:")}</h4>
                    <p className="text-xs text-slate-200 leading-relaxed">{ui(" “To complete your subscription, contact our support team to receive the USDT TRC20 payment address and payment instructions.” ")}</p>
                    <div className="p-2.5 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                      <p className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                        <CheckCircle2 className="w-4 h-4 shrink-0" />
                        <span>{ui("Instant verification — subscription will be activated after confirmation.")}</span>
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
                  ...
                  <div className="shrink-0 flex flex-col items-center bg-white p-2.5 rounded-2xl shadow-lg border border-slate-200">
                    <img
                      ...
                      alt={ui("SM Trading Pro Official Telegram Support QR Code")}
                      className="w-32 h-32 object-contain"
                    />
                    <span className="text-[9px] font-bold text-slate-700 mt-1 text-center font-mono">{ui(" SCAN VIA TELEGRAM ")}</span>
                  </div>

                  {/* Desk Info & Primary CTA */}
                  <div className="min-w-0 flex-1 text-center sm:text-left rtl:sm:text-right space-y-3">
                    <div>
                      <span className="text-[11px] font-bold text-blue-400 uppercase tracking-wider block">{ui(" Official SM Trading Pro Support Desk ")}</span>
                      ...
                      <p className="text-xs text-slate-300 mt-1">{ui(" Active 24/7 for instant USDT payment instructions, transaction validation, and member onboarding. ")}</p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1">
                      ...
                      >
                        ...
                        <span>{ui("Contact Support for Payment Details")}</span>
                      ...
                      <button
                        ...
                        title={ui("Copy support handle")}
                      >
                        {copiedHandle ? (
                          <>
                            <Check className="w-3.5 h-3.5 text-emerald-400" />
                            <span className="text-emerald-400">{ui("Copied!")}</span>
                          </>
                        ) : (
                          <>
                            <Copy className="w-3.5 h-3.5 text-slate-400" />
                            <span>{ui("Copy Handle")}</span>
                          </>
                        )}
                      </button>
                    </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
                      ...
                      <span>{ui("Submit Payment Confirmation / TxID")}</span>
                    </h4>
                    <p className="text-xs text-slate-400 mt-0.5">{ui(" Already sent your USDT transfer or messaging support? Submit your transaction details below for immediate manual verification and credential dispatch. ")}</p>
                  </div>
                </div>
                ...
                {submitSuccess ? (
                  <div className="p-4 bg-emerald-950/50 border border-emerald-500/50 rounded-xl space-y-3">
                    <div className="flex items-center gap-2 text-emerald-400 font-bold text-sm">
                      ...
                      <span>{ui("Payment Confirmation Received!")}</span>
                    </div>
                    <p className="text-xs text-slate-200 leading-relaxed">{ui(" Your order reference is ")}<strong className="font-mono text-emerald-300">{orderId}</strong>{ui(". Our support desk has received your submission. ")}</p>
                    <div className="pt-2">
                      <a
                      ...
                      >
                        <Send className="w-3.5 h-3.5" />
                        <span>{ui("Notify Support on Telegram")}</span>
                      </a>
                    </div>
                  </div>
                ) : (
                  <form onSubmit={handleSubmitConfirmation} className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[11px] font-semibold text-slate-300 mb-1">{ui(" Your Telegram Username ")}<span className="text-amber-400">*</span>
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[11px] font-semibold text-slate-300 mb-1">{ui(" Your Email (For Account Credentials) ")}<span className="text-amber-400">*</span>
                        </label>
                        <input
                          type="email"
                          value={email}
                    ...
                    <div>
                      <label className="block text-[11px] font-semibold text-slate-300 mb-1">{ui("USDT (TRC20) Transaction Hash / TxID (Or \"Pending Transfer with Support\")")}</label>
                      <input
                        type="text"
                        value={txHash}
                        onChange={(e) => setTxHash(e.target.value)}
                        placeholder={ui("e.g. TRC20 Transaction Hash (or Pending with Support)")}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-[11px] font-semibold text-slate-300">{ui(" Referral / Affiliate Code (Optional) ")}</label>
                        {referralCode && (
                          <span className="text-[10px] text-emerald-400 font-mono font-bold flex items-center gap-1">
                            <Check className="w-3 h-3" />{ui(" Applied ")}</span>
                        )}
                      </div>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-[11px] font-semibold text-slate-300 mb-1">{ui(" Additional Notes (Optional requested username, questions) ")}</label>
                      <textarea
                        value={notes}
                        onChange={(e) => setNotes(e.target.value)}
                        rows={2}
                        placeholder={ui("Any additional instructions or requested username...")}
                    ...
                    >
                      {isSubmitting ? (
                        <span>{ui("Submitting Confirmation...")}</span>
                      ) : (
                        <>
                          <CheckCircle2 className="w-4 h-4" />
                          <span>{ui("Submit Payment Details for Verification")}</span>
                        </>
                      )}
                    </button>
                  </form>
        ...
        <div className="p-3 sm:p-4 bg-[#0A0E1A] border-t border-slate-800 flex items-center justify-between text-xs text-slate-400 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span>{ui("Support online on Telegram: ")}<strong>@SMTrading_SUPPORT</strong></span>
          </div>
          <button
            type="button"
            onClick={onClose}
