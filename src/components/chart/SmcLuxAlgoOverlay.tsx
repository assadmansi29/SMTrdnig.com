        ...
        <div className="flex items-center gap-1.5 bg-[#0C111D]/90 hover:bg-[#0E1524] backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-slate-800/90 shadow-xl transition-all duration-200">
          ...
          {settings.enabled && analysis && (
            <div
              ...
              title={ui("Live Market Structure: {p0}", {p0: isBullish ? 'Bullish Trend' : 'Bearish Trend'})}
            >
              <span
              ...
              {isBullish ? (
                <div className="flex items-center gap-0.5">
                  ...
                  <span>{ui("BULLISH")}</span>
                </div>
              ) : (
                <div className="flex items-center gap-0.5">
                  <TrendingDown className="w-2.5 h-2.5" />
                  <span>{ui("BEARISH")}</span>
                </div>
              )}
            </div>
          )}
          ...
          {settings.enabled && analysis && (
            <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-mono text-slate-400 pl-1 border-l border-slate-800">
              <span title={ui("Active Unmitigated Order Blocks")}>OB: <strong className="text-slate-200">{activeObs}</strong></span>
              <span title={ui("Unfilled Fair Value Gaps")}>FVG: <strong className="text-slate-200">{activeFvgs}</strong></span>
              {lastSignal && (
                <span
                  className={
                    lastSignal.direction === 'bullish' ? 'text-emerald-400' : 'text-rose-400'
                  }
                  title={ui("Latest Structure Break: {p0}", {p0: lastSignal.type})}
                >
                  {lastSignal.type}
                </span>
              )}
          ...
          {canEdit && <div className="flex items-center gap-0.5 pl-1 border-l border-slate-800">
            <button
              ...
              title={settings.enabled ? ui("Hide SMC Overlay") : ui("Show SMC Overlay")}
            >
        ...
        {isExpanded && settings.enabled && analysis && (
          <div className="p-2 rounded-xl bg-[#090D17]/95 border border-slate-800 shadow-2xl backdrop-blur-md max-w-xs text-[11px] space-y-1.5 animate-in fade-in slide-in-from-top-1">
            <div className="flex items-center justify-between text-slate-400 pb-1 border-b border-slate-800">
              <span className="font-semibold text-slate-200">{ui("Smart Money Breakdown")}</span>
              <span className="text-[10px] font-mono text-sky-400">{ui("Sens: ")}{settings.swingLength}{ui(" bars")}</span>
            </div>
            <div className="grid grid-cols-2 gap-2 text-slate-300">
              <div className="bg-[#121A2B] p-1.5 rounded-lg border border-slate-800/80">
                <span className="text-slate-400 block text-[10px]">{ui("Order Blocks")}</span>
                <span className="font-bold font-mono text-emerald-400">{activeObs}{ui(" Active")}</span>
              </div>
              <div className="bg-[#121A2B] p-1.5 rounded-lg border border-slate-800/80">
                <span className="text-slate-400 block text-[10px]">{ui("Fair Value Gaps")}</span>
                <span className="font-bold font-mono text-cyan-400">{activeFvgs}{ui(" Active")}</span>
              </div>
            </div>
