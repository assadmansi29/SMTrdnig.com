...
for(const [symbol,entry] of [['OANDA:XAUUSD',2345.2],['OANDA:EURUSD',1.1],['OANDA:USDJPY',150],['OANDA:WTICOUSD',70],['OANDA:NAS100USD',20000],['OANDA:US30USD',40000],['BINANCE:BTCUSDT',60000]] as const)for(const side of [-1,1]){
 ...
 openReactionTrade(signal,symbol,'fib','zone');const original=getReactionTrades()[0];assert.ok(Math.abs(Math.abs(original.stop-entry)/u.size-30)<1e-6);
 ...
 openReactionTrade({...signal,id:signal.id+'stop'},symbol,'smc','zone');updateReactionTradePrice(symbol,quote(-30),now+7000);assert.equal(getReactionTrades().length,0,'original stop before TP1');
