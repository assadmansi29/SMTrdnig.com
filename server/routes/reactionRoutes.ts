...
router.get('/snapshot',(req:AuthRequest,res)=>res.json(visibleReactionSnapshot(reactionAuthority.snapshot())));
router.get('/stream',(req:AuthRequest,res)=>{
  ...
  const send=(snapshot:ReturnType<typeof reactionAuthority.snapshot>)=>{
    ...
    res.write(`id: ${snapshot.epoch}:${snapshot.sequence}\ndata: ${JSON.stringify(visibleReactionSnapshot(snapshot))}\n\n`);
  };
