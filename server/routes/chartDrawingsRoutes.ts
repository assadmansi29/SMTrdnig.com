...
router.get('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Match symbol across broker prefixes (e.g. XAUUSD, OANDA:XAUUSD, FX:XAUUSD)
    const baseTicker = symbol.includes(':') ? symbol.split(':')[1] : symbol;
    const oandaSymbol = `OANDA:${baseTicker}`;
    const blackbullSymbol = `BLACKBULL:${baseTicker}`;
    ...
    const query = `
      ...
      AND (
        strategy = $5 
        OR ($5 = 'default' AND (strategy IS NULL OR strategy = 'default' OR strategy = ''))
      )
      ORDER BY created_at ASC
    `;

    const result = await pool.query(query, [symbol, baseTicker, oandaSymbol, blackbullSymbol, strategy]);
...
async function handleBatchSave(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    try {
      ...
      const placeholders = validIds.map((_, i) => `$${i + 6}`).join(', ');
      await client.query(
        ...
        [symbol, baseTicker, oandaSymbol, blackbullSymbol, strategy, ...validIds]
      );
...
router.post('/delete-strategy', authenticateToken, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      ...
      [symbol, baseTicker, oandaSymbol, blackbullSymbol, strategy]
    );
